...
export function inferModelCapability(model: string): LogicalModelCapability {
    ...
    if (/(?:^|[-_.\s/])(image|images|img|flux|sdxl|midjourney|banana[-_.\s]?2)(?:$|[-_.\s/])|nano[-_.\s]?banana|banana[-_.\s]?pro|seedream|gpt[-_.]?image|dall[-_.]?e|imagen|stable[-_.\s]?diffusion/.test(value) || isShortStableDiffusionName(value)) return "image";
    return "text";
}

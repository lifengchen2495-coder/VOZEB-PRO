...
export function frameRemakeWorkflowReadiness(project: FrameRemakeProject) {
    const analysis = project.groups.length > 0 && project.groups.every((group) => group.contactSheet && group.frames.every((frame) => frame.media) && group.analysis);
    const images =
...
export function nextFrameRemakeStep(project: FrameRemakeProject) {
    ...
    if (frames) return { kind: "extract" as const, workflowStage: "analysis" as const, groupId: frames.id, label: `第 ${frames.number} / ${project.groups.length} 组：拆帧` };
    for (const group of project.groups) {
        if (!group.analysis) return { kind: "analyze" as const, workflowStage: "analysis" as const, groupId: group.id, analysisStage: "analysis" as const, label: `第 ${group.number} / ${project.groups.length} 组：分析原片画面` };
    }
    for (const group of project.groups) {

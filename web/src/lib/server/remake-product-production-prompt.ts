import { REMAKE_FEISHU_VIDEO_PROMPTS } from "@/lib/remake-product-feishu-prompts";
    ...
    return [
        ...
        },
        {
            ...
            content: JSON.stringify({
                "人物规则": "严格保留来源十二宫格中的原人物身份、服装、背景和动作，不提供替换人物图",
                "产品信息": input.productInfo,
                [`分镜${groupId}生图（十二宫格图）`]: {
...
export function assertRemakeVideoPrompt(value: string, input: Pick<RemakeProductionPromptInput, "copyBlocks" | "hasNarration" | "voice">, groupId: RemakeProductionGroupId) {
    if (value.includes("@人物图")) throw new Error("换品不换人提示词不能引用人物替换图");

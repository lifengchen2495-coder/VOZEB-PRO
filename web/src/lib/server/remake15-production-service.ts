...
import { isRemakeNoNarrationCopy } from "@/lib/server/remake15-project-contract";
import { assertRemakeVideoPrompt, remakeProductionMessages, REMAKE_PRODUCTION_GROUP_IDS, renderRemakeCopyReport, type RemakeProductionPromptInput } from "@/lib/server/remake15-production-prompt";
import { assertRemakeImageGenerationsForUser, completeRemakeProductionForUser, getRemakeProjectForUser, remakeProductionInputVersion, RemakeProjectServiceError } from "@/lib/server/remake15-project-service";
import { buildRemakeProductionVisualBoards, RemakeProductionVisionError, requestRemakeProductionVisionPrompt, resolveRemakeProductionVisionProtocol } from "@/lib/server/remake15-production-vision-runtime";
...
const runtime = globalThis as typeof globalThis & { __vozebProRemake15Productions?: Map<string, PendingProduction> };
...
export async function buildRemakeProductionForUser(input: RemakeProductionRequest) {
    ...
    const pending = runtime.__vozebProRemake15Productions ??= new Map();
    ...
    try {
        for (const groupId of selectedGroups) {
            ...
            for (const candidate of candidates) {
                const idempotencyKey = systemAiIdempotencyKey("remake15-feishu-video-prompt", input.userId, project.id, remakeProductionInputVersion(project, [groupId]), groupId, candidate.channelId, candidate.upstreamModel);
...
async function assertProductionReady(userId: string, project: Awaited<ReturnType<typeof getRemakeProjectForUser>>) {
    if (!project.productScript.trim() || !project.storyboardScript.trim()) throw new RemakeProductionError("请先生成新产品脚本和 1-12 分镜提示词", 409);
    if (project.analysis.status !== "completed" || project.analysis.mode !== "video" || project.frames.length !== 12 || project.frames.some((frame, index) => frame.ordinal !== index + 1 || frame.analysisStatus !== "available")) {
        throw new RemakeProductionError("请先使用视频理解完成全部 12 个镜头解析", 409);
    }
    const hasNarration = !isRemakeNoNarrationCopy(project.sourceCopy);
    if (project.copyBlocks.length !== 4 || project.copyBlocks.some((block, index) => block.ordinal !== index + 1 || (hasNarration ? !block.sourceText.trim() || !block.text.trim() : Boolean(block.sourceText.trim() || block.text.trim())))) {
        throw new RemakeProductionError("请先完成 4 个语义文案区间", 409);
    }
    ...
    try {
        ...
        throw new RemakeProductionError(error instanceof Error ? error.message : "4 个文案区间未完整覆盖原文案", 409);
    }
    ...
    if (project.groups.length !== 1 || project.groups.some((group) => !group.sourceContactSheet || group.imageGeneration.status !== "completed" || !group.imageGeneration.result)) {
        throw new RemakeProductionError("请先完成一组清理换人和换品十二宫格生图", 409);
    }

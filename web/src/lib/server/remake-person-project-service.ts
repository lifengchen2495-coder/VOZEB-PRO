...
import { createRemakeAnalysisTask, failRemakeAnalysisTask, findActiveRemakeAnalysisTask, getRemakeAnalysisTask, queueRemakeAnalysisTask, type RemakeAnalysisTask } from "@/lib/server/remake-person-analysis-task-store";
import { remakeContactSheetDimensionError } from "@/lib/server/remake-contact-sheet-validation";
import { createRemakeProject, deleteRemakeProject, getRemakeProject, listRemakeProjectSummaries, mutateRemakeProject, RemakeProjectStoreError, updateRemakeProject } from "@/lib/server/remake-person-project-store";
...
function authoritativeImageAsset(task: ImageTask, groupId: string): RemakeMediaAsset | undefined {
    ...
    return {
        ...
        originalName: `remake-person-${groupId}.png`,
        bytes: result?.bytes,
        width: result?.width,
...
function remakeImageGenerationSlotId(stage: "storyboard", groupId: string) {
    return `remake-person:${groupId}:${stage}`;
}
...
async function authoritativeRemakeVideoGeneration(input: {
    ...
    if (!task || task.userId !== input.userId || task.projectId !== input.projectId || task.generationSlotId !== `remake-person-video:${input.group.id}` || task.prompt !== input.group.videoPrompt) {

...
async function mergeAndPersist(input: MergeRequest, project: HydratedRemakeProject, version: string) {
    ...
    let committed = false;
    try {
        workDirectory = await mkdtemp(join(tmpdir(), "vozeb-remake-product-merge-"));
        const normalizedFiles: string[] = [];
        for (const group of project.groups) {
            const task = await getVideoTask(group.videoGeneration.taskId!);
            if (!task || task.userId !== input.userId || task.projectId !== project.id || task.generationSlotId !== `remake-product-video:${group.id}` || task.status !== "success" || task.requestedDurationSeconds !== 15 || task.result?.url !== group.videoGeneration.result?.url) throw new RemakeProjectServiceError(`分镜 ${group.id} 的视频任务与当前成片不一致`, 409);
        ...
        const asset = await writeAssetBytes(await readFile(output), "video/mp4", "video", { ownerUserId: input.userId, source: "remake-product-merge", taskId: `${project.id}:merged`, originalName: `${project.title}-1分钟.mp4` });

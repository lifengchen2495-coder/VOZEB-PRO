...
async function mergeAndPersist(input: MergeRequest, project: HydratedRemakeProject, version: string) {
    ...
    try {
        ...
        for (const group of project.groups) {
            ...
            if (!task || task.userId !== input.userId || task.projectId !== project.id || task.generationSlotId !== `remake-person-video:${group.id}` || task.status !== "success" || task.requestedDurationSeconds !== 15 || task.result?.url !== group.videoGeneration.result?.url) throw new RemakeProjectServiceError(`分镜 ${group.id} 的视频任务与当前成片不一致`, 409);

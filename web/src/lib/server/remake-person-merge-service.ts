...
async function mergeAndPersist(input: MergeRequest, project: HydratedRemakeProject, version: string) {
    ...
    try {
        ...
        for (const group of project.groups) {
            ...
            await runFfmpeg(remakeMergeNormalizationArgs(source, join(workDirectory, name), hasAudio, project.videoSettings, timing), { timeoutMs: 10 * 60_000 });
        ...
        created = { url: asset.serverUrl || asset.url, storageKey: localMediaStorageKeyFromValue(asset.serverUrl || asset.url), mimeType: "video/mp4", originalName, bytes: asset.bytes, durationMs: Math.round(delivered.duration * 1000), ...remakeVideoOutputDimensions(project.videoSettings) };
...
export function remakeMergeNormalizationArgs(source: string, output: string, hasAudio: boolean, settings: RemakeVideoSettings | undefined, timing: RemakePersonTiming) {

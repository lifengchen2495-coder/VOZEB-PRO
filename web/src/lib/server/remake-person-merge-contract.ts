...
export function remakeMergeInputVersion(project: Pick<RemakeProject, "id" | "groups" | "videoSettings">) {
    const settings = remakeVideoSettingsKey(project.videoSettings);
    return createHash("sha256").update(JSON.stringify({ id: project.id, ...(settings ? { videoSettings: settings } : {}), groups: project.groups?.map((group) => ({ id: group.id, videoPrompt: group.videoPrompt, status: group.videoGeneration.status, taskId: group.videoGeneration.taskId || "", attemptNo: group.videoGeneration.attemptNo ?? 0, model: group.videoGeneration.model || "", url: group.videoGeneration.result?.url || "" })) })).digest("hex");
}

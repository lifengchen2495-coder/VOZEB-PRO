...
import type { RemakeCopyBlock, RemakeFrame, RemakeProject } from "@/lib/server/remake-product-project-contract";
...
import { getRemakeProjectForUser } from "@/lib/server/remake-product-project-service";
...
export async function handoffRemakeProjectToDrama(userId: string, projectId: string) {
    ...
    const sourceHandoffId = `remake-product-${project.id}`;
...
export function buildRemakeDramaCreateInput(project: RemakeProject, sourceHandoffId = `remake-product-${project.id}`) {
...
function buildShot(project: RemakeProject, block: RemakeCopyBlock, frames: RemakeFrame[], first: RemakeFrame, last: RemakeFrame, index: number): DramaShot {
    ...
    return {
        id: `remake-product-shot-${project.id}-${block.ordinal}`,
        order: index + 1,
        ...
        utterances: copy ? [{ id: `remake-product-utterance-${project.id}-${block.ordinal}`, order: 1, type: "voiceover", speaker: voiceover ? "旁白" : "", text: copy }] : [],
        ...
        duration,
        characterIds: voiceover ? ["remake-product-narrator"] : [],
        propIds: [],
        clueIds: [],
...
function buildSourceAssets(project: RemakeProject): DramaSourceAsset[] {
    ...
    if (project.sourceVideo?.url) {
        assets.push({
            id: `remake-product-source-video-${project.id}`,
            type: "video",
    ...
    if (project.sourceCopy.trim()) assets.push({ id: `remake-product-source-copy-${project.id}`, type: "text", title: "原文案", textContent: project.sourceCopy });
    return assets;
}

function narratorCharacter(voice: RemakeProject["voice"]): DramaCharacter {
    return {
        id: "remake-product-narrator",
        name: "旁白",

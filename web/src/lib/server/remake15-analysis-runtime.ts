...
async function understandVideo(input: {
    ...
    throw new Error(toSafeGenerationErrorMessage(latestError, "视频理解模型未返回完整的 12 条镜头分析"));
}
...
async function planSemanticCopy(input: {
    ...
    throw new Error(toSafeGenerationErrorMessage(latestError, "文案模型未返回完整的 4 段语义切分"));
}

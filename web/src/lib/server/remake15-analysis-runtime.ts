...
import { REMAKE_FEISHU_ANALYSIS_PROMPT, REMAKE_FEISHU_COPY_PROMPT } from "@/lib/remake15-feishu-prompts";
...
import { completeRemakeAnalysisTask, failRemakeAnalysisTask, markRemakeAnalysisTaskRunning, updateRemakeAnalysisTaskProgress, type RemakeAnalysisTask } from "@/lib/server/remake15-analysis-task-store";
import {
...
import { completeRemakeProjectAnalysis, failRemakeProjectAnalysis, markRemakeProjectAnalysisRunning, RemakeAnalysisSupersededError } from "@/lib/server/remake15-project-service";
...
export async function runRemakeAnalysisTask(input: { task: RemakeAnalysisTask; origin: string; cookie: string }) {
    ...
    try {
        ...
        workDirectory = await mkdtemp(join(tmpdir(), "vozeb-remake15-analysis-"));
...
async function extractReferenceAudio(input: { sourcePath: string; workDirectory: string; task: RemakeAnalysisTask; onAsset: (storageKey: string) => void }): Promise<RemakeMediaAsset> {
    ...
    const stored = await writeReferenceMediaFile(outputPath, "audio", "audio/aac", true, {
        ...
        source: "remake15-analysis-audio",
        originalName: "remake15-source-voice.aac",
...
async function understandVideo(input: {
    ...
    for (const candidate of input.candidates) {
        const idempotencyKey = systemAiIdempotencyKey("remake15-video-understanding", input.task.userId, input.task.id, candidate.channelId, candidate.upstreamModel);
        try {
    ...
    throw new Error(`来源视频理解失败：${toSafeGenerationErrorMessage(latestError, "视频理解模型未返回完整的 12 条镜头分析")}`);
}
...
function buildDoubaoVideoUnderstandingPrompt(durationMs: number) {
    const runtimeContract = [
        "## 【当前记录执行合同】",
        "完整观看已上传的视频，并按时间顺序严格拆解为恰好 12 个编号分镜，一部分共 12 个。",
        "12 个分镜必须完整、连续覆盖视频：第一段从 0:00 开始，后一段的开始时间必须等于前一段的结束时间，禁止时间跳跃、重叠、重复或乱序；最后一段结束时间必须等于视频实际结尾。",
...
async function uploadDoubaoVideo(candidate: ResolvedLogicalModel, bytes: Buffer) {
    const upload = buildDoubaoFileUploadBody(bytes, "remake15-analysis-video.mp4");
...
async function extractAnalyzedFrames(input: { sourcePath: string; workDirectory: string; analysis: VideoFrameAnalysis[]; task: RemakeAnalysisTask; onAsset: (storageKey: string) => void }): Promise<ExtractedFrame[]> {
    ...
    const persisted = await mapConcurrent(samples, FRAME_PERSISTENCE_CONCURRENCY, async (sample) => {
        ...
        const asset = await writeAssetBytes(bytes, "image/jpeg", "image", {
            ownerUserId: input.task.userId,
            source: "remake15-analysis",
            taskId: input.task.id,
            originalName: `remake15-frame-${String(sample.ordinal).padStart(2, "0")}.jpg`,
...
async function createAndPersistContactSheets(input: { frames: ExtractedFrame[]; task: RemakeAnalysisTask; onAsset: (storageKey: string) => void }): Promise<RemakeContactSheetInput[]> {
    ...
    return mapConcurrent(batches, 2, async (batch, batchIndex) => {
        ...
        const originalName = `remake15-contact-sheet-${String(groupOrdinal).padStart(2, "0")}.jpg`;
        const asset = await writeAssetBytes(bytes, "image/jpeg", "image", {
            ownerUserId: input.task.userId,
            source: "remake15-analysis",
            taskId: input.task.id,
...
async function planSemanticCopy(input: {
    ...
    let latestError: unknown;
    for (const candidate of input.candidates) {
        const baseKey = systemAiIdempotencyKey("remake15-copy-planning", input.task.userId, input.task.id, candidate.channelId, candidate.upstreamModel);
    ...
    throw new Error(`原文案切分失败：${toSafeGenerationErrorMessage(latestError, "文案模型未返回完整的 4 段语义切分")}`);
}
...
function buildCopyPlan(segmentation: CopySegmentation, frames: ExtractedFrame[]): CopyPlan {
    if (segmentation.segments.length !== REMAKE_COPY_BLOCK_COUNT || segmentation.texts.length !== REMAKE_COPY_BLOCK_COUNT || segmentation.paragraphOrdinalsByBlock.length !== REMAKE_COPY_BLOCK_COUNT || frames.length !== REMAKE_FRAME_COUNT) {
        throw new Error("无法生成完整的 4 个文案区间");
    }
    ...
    if (hasNarration && blocks.some((block) => !block.sourceText.trim() || !block.text.trim())) throw new Error("文案模型必须返回完整的 4 个非空文案区间");
...
function renderFeishuCopyReport(blocks: RemakeCopyBlock[], paragraphs: Array<{ ordinal: number; text: string }>, mappings: Array<{ blockOrdinal: number; paragraphOrdinals: number[] }>, statuses: CopyBlockStatus[], optionRaw: string) {
    ...
    const countStatus = (status: CopyBlockStatus) => statuses.filter((value) => value === status).length;
    const numerals = ["一"];
    const groups = Array.from({ length: 1 }, (_, groupIndex) => {
        const rows = blocks
...
function trackAnalysisCharge(pendingRefunds: Map<string, PendingAnalysisRefund>, task: RemakeAnalysisTask, model: string, stage: "video-understanding" | "copy-planning", headers: Headers) {
    ...
    pendingRefunds.set(billing.pointsRecordId, {
        ...
        idempotencyKey: systemAiIdempotencyKey("remake15-analysis-refund", task.userId, task.id, task.runId, stage, billing.pointsRecordId),
    });
}
...
const remakeVideoTool = {
    ...
    description: "完整分析电商视频，按视频原语言逐字返回口播原文及固定 12 个镜头单元",
    parameters: {
        type: "object",
        properties: {
...
const remakeCopyTool = {
    ...
    description: "保留原文切片，并依据 12 帧上下文输出仅补全字幕或校对明显 ASR 错误后的 4 个最终文案区间",
    parameters: {
        type: "object",
        properties: {
            paragraphs: {

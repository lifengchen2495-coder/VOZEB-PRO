...
export function resolveRemakeProductionVisionProtocol(candidate: ResolvedLogicalModel, requiredImageCount = 2): ResolvedVisionProtocol | null {
    if (requiredImageCount > 0 && candidate.capabilityProfile?.supportsReferenceImage !== true) return null;
    if (candidate.capabilityProfile?.maxReferenceImages !== undefined && candidate.capabilityProfile.maxReferenceImages < requiredImageCount) return null;
    try {
...
export async function requestRemakeProductionVisionPrompt(input: {
    ...
    const protocol = resolveRemakeProductionVisionProtocol(input.candidate, input.boards.length);
...
async function createContactSheetBoard(images: LoadedImage[], groups: Array<{ groupOrdinal: number; frameOrdinals: number[] }>): Promise<RemakeProductionVisualBoard> {
    ...
    return {
        ...
        bytes,
        description: "四张重绘十二宫格，按左上、右上、左下、右下对应四组，内部按 3×4 从左到右、从上到下对应连续 12 帧。",

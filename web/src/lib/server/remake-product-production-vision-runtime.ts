...
export async function buildRemakeProductionVisualBoards(input: {
    ...
    const referenceBoard = await createReferenceBoard(product);
...
async function createReferenceBoard(product: LoadedImage): Promise<RemakeProductionVisualBoard> {
    const overlays: OverlayOptions[] = [
        { input: await fitImage(product.bytes, REFERENCE_BOARD_WIDTH, REFERENCE_BOARD_HEIGHT - LABEL_HEIGHT), left: 0, top: LABEL_HEIGHT },
        { input: labelSvg(REFERENCE_BOARD_WIDTH, LABEL_HEIGHT, "PRODUCT"), left: 0, top: 0 },
    ];
    const bytes = await renderBoard(REFERENCE_BOARD_WIDTH, REFERENCE_BOARD_HEIGHT, overlays);
    return {
        ordinal: 1, id: "reference-board", mimeType: "image/jpeg",
        width: REFERENCE_BOARD_WIDTH, height: REFERENCE_BOARD_HEIGHT, bytes,
        description: "新产品图；原人物、服装与场景以四组十二宫格为准，严格保持不变。",
        layout: [{ order: 1, role: "product", label: "PRODUCT", position: "center", provided: true }],
    };
}

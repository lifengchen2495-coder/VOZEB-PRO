...
import { renderRemakeCopyReport } from "./remake-production-prompt";
...
async function authoritativeRemakeImageGeneration(input: {
    ...
    if (task.status === "pending" || task.status === "running") return { ...resolveRemakeActiveImageTaskState({ taskId: task.id, requested: input.requested, execution: task }), taskId: task.id, model: taskModel, prompt };
...
function authoritativeImageAsset(task: ImageTask, groupId: string): RemakeMediaAsset | undefined {
    ...
    return {
        ...
        originalName: `remake-${groupId}.png`,
        bytes: result?.bytes,
        width: result?.width,

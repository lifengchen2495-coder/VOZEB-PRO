...
export const REMAKE_PRODUCTION_GROUP_COUNT = 1;
...
export const REMAKE_PRODUCTION_GROUP_IDS: readonly RemakeProductionGroupId[] = ["1-12"];
    ...
    return [
        ...
        },
        {
            ...
            content: JSON.stringify({
                ...
                "1-12分镜提示词": input.storyboardScript,
                "原文案（可选）": input.sourceCopy,
                "文案预处理": {
...
export function renderRemakeCopyReport(input: RemakeProductionCopyReportInput) {
    ...
    if (blocks.length !== 4 || blocks.some((block, index) => block.ordinal !== index + 1)) throw new Error("文案预处理必须包含连续的 4 个区间");
    ...
    const sections = Array.from({ length: 1 }, (_, groupIndex) => {
    ...
    return fenced(
        [
            ...
            "- 分镜区间数量：4个",
            `- 平均每区间对应段落：约${(paragraphs.length / 4).toFixed(1)}个`,
            "",
            "段落分配表：",
            "| 区间 | 分镜范围 | 文案段落 |",
            "|---|---|---|",
...
export function assertRemakeCopyCoverage(sourceCopy: string, blocks: RemakeProductionCopyBlock[]) {
    ...
    if (blocks.some((block) => !block.sourceText.trim() || !block.text.trim())) throw new Error("文案预处理必须包含连续且非空的 4 个区间");
    const assigned = [...blocks]
        .sort((left, right) => left.ordinal - right.ordinal)
        .map((block) => block.sourceText)
        .join("");
    if (sourceCopy !== assigned) throw new Error("4 个文案区间没有按原顺序完整覆盖原文案（必须逐字符一致）");
}
...
function assertSemanticCopyData(sourceCopy: string, blocks: RemakeProductionCopyBlock[], paragraphs: Array<{ ordinal: number; text: string }>, mappings: Array<{ blockOrdinal: number; paragraphOrdinals: number[]; sourceText: string; text: string }>) {
    if (!sourceCopy.trim()) {
        if (paragraphs.length || mappings.length !== 4 || mappings.some((mapping, index) => mapping.blockOrdinal !== index + 1 || mapping.paragraphOrdinals.length || mapping.sourceText.trim() || mapping.text.trim())) {
    ...
    if (mappings.length !== 4 || mappings.some((mapping, index) => mapping.blockOrdinal !== index + 1 || !mapping.paragraphOrdinals.length)) {
        throw new Error("文案预处理必须保留连续的 4 个段落映射");
    }
    ...
    if (
        ...
        throw new Error("文案段落映射与 4 个口播区间不一致");
    }
}

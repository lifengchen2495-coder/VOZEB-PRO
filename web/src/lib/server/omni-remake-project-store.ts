import type { OmniProject as RemakeProject, OmniProjectList as RemakeProjectSummaryPage } from "@/lib/omni-remake-contract";
const summarizeRemakeProject = (project: RemakeProject) => project;
...
const FILE_NAME = "omni-remake-projects.json";
...
export async function getRemakeProject(id: string, userId: string): Promise<RemakeProject | null> {
    if (getDatabaseProvider() === "postgres") {
        await ensurePostgresSchema();
        const result = await postgresQuery<{ project_json: RemakeProject }>("SELECT project_json FROM omni_remake_projects WHERE id = $1 AND user_id = $2", [id, userId]);
...
export async function createRemakeProject(userId: string, project: RemakeProject) {
    if (getDatabaseProvider() === "postgres") {
        ...
        const result = await postgresQuery(
            `INSERT INTO omni_remake_projects (id, user_id, title, status, project_json, created_at, updated_at)
...
export async function mutateRemakeProject(userId: string, id: string, mutate: (current: RemakeProject) => RemakeProject | null): Promise<RemakeProject | null> {
    if (getDatabaseProvider() === "postgres") {
        ...
        return withPostgresTransaction(async (client) => {
            const selected = await client.query<{ project_json: RemakeProject }>("SELECT project_json FROM omni_remake_projects WHERE id = $1 AND user_id = $2 FOR UPDATE", [id, userId]);
...
export async function deleteRemakeProject(userId: string, id: string) {
    if (getDatabaseProvider() === "postgres") {
        await ensurePostgresSchema();
        const result = await postgresQuery<{ project_json: RemakeProject }>("DELETE FROM omni_remake_projects WHERE id = $1 AND user_id = $2 RETURNING project_json", [id, userId]);
        return result.rows[0]?.project_json || null;
    }
    return withFileMutation((database) => {

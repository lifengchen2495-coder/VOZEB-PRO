...
import { REMAKE_FEISHU_PRODUCT_SCRIPT_PROMPT, REMAKE_FEISHU_STORYBOARD_SCRIPT_PROMPT } from "@/lib/remake15-feishu-prompts";
import { toSafeGenerationErrorMessage } from "@/lib/server/generation-errors";
import { resolveLogicalModelCandidates } from "@/lib/server/logical-model-router";
import { completeRemakeProductScriptsForUser, getRemakeProjectForUser, RemakeProjectServiceError } from "@/lib/server/remake15-project-service";
import { buildRemakeScriptVisualBoards, RemakeProductionVisionError, requestRemakeProductionVisionPrompt, resolveRemakeProductionVisionProtocol } from "@/lib/server/remake15-production-vision-runtime";
...
export async function buildRemakeProductScriptsForUser(input: ScriptRequest) {
    ...
    if (project.analysis.status !== "completed" || project.analysis.mode !== "video" || project.frames.length !== 12 || project.frames.some((frame, index) => frame.ordinal !== index + 1 || frame.analysisStatus !== "available")) throw new RemakeProjectServiceError("请先完成 12 镜头解析", 409);
    if (!project.productInfo.trim() || !project.references.product?.url) throw new RemakeProjectServiceError("请先填写新产品信息并上传产品图", 409);
    if (!project.groups[0]?.sourceContactSheet?.url) throw new RemakeProjectServiceError("原视频十二宫格尚未完成", 409);
...
async function generateScripts(input: ScriptRequest, project: Awaited<ReturnType<typeof getRemakeProjectForUser>>) {
    ...
    async function generate(stage: "product-script" | "storyboard-script", systemPrompt: string, context: Record<string, unknown>, references: Parameters<typeof buildRemakeScriptVisualBoards>[0]["references"]) {
        const boards = await buildRemakeScriptVisualBoards({ origin: input.origin, cookie: input.cookie, references });
        let latestError: unknown;
        for (const candidate of candidates) {
            const key = systemAiIdempotencyKey("remake15-script", input.userId, project.id, String(project.revision), stage, candidate.channelId, candidate.upstreamModel);
            ...
            try {
                ...
                assertRemakeTwelveShotScript(call.text);
        ...
        throw new RemakeProjectServiceError(toSafeGenerationErrorMessage(latestError, "未能生成完整的 12 分镜脚本"), 422);
    }
    try {
        const productScript = await generate("product-script", REMAKE_FEISHU_PRODUCT_SCRIPT_PROMPT, {
            "产品信息": project.productInfo,
            "12镜头解析": project.frames,
        ...
        const storyboardScript = await generate("storyboard-script", REMAKE_FEISHU_STORYBOARD_SCRIPT_PROMPT, {
            "新产品-12分镜脚本": productScript,
        }, [{ role: "redrawn-contact-sheet", label: "1-12拼图（原视频十二宫格）", asset: project.groups[0].sourceContactSheet! }]);
        ...
        // 两个脚本作为一个结果交付，失败或素材被替换时退回本次已完成调用的费用。
...
export function assertRemakeTwelveShotScript(value: string) {
    ...
    if (ordinals.length !== 12 || ordinals.some((ordinal, index) => ordinal !== index + 1)) throw new Error("分镜脚本必须依次包含分镜 1 至分镜 12，不能缺失、重复或乱序");
}

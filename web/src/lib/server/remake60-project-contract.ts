...
export type RemakeProject = {
    ...
    mergedVideo?: RemakeMediaAsset;
    mergedVideoInputVersion?: string;
...
export function normalizeRemakeProjectWorkflow(project: RemakeProject): HydratedRemakeProject {
    ...
    if (project.sourceCopy.trim() === REMAKE_NO_NARRATION_TEXT) copy.optionRaw = REMAKE_NO_NARRATION_TEXT;
    ...
    return {
        ...project,
        sourceCopy,
        productInfo: cleanText(project.productInfo, 20_000),
        productScript: cleanText(project.productScript, 100_000),
        storyboardScript: cleanText(project.storyboardScript, 100_000),
        mergedVideo: normalizeRemakeMediaAsset(project.mergedVideo),
        mergedVideoInputVersion: cleanText(project.mergedVideoInputVersion, 64) || undefined,
        frames,
        copyBlocks,
        analysis: {
            ...project.analysis,
...
export function remakeSourceCopyForAnalysis(project: Pick<RemakeProject, "sourceCopy" | "copy">, detectedCopy: string) {
    if (project.sourceCopy.trim() === REMAKE_NO_NARRATION_TEXT || project.copy?.optionRaw === REMAKE_NO_NARRATION_TEXT) return "";
    return project.sourceCopy.trim() ? project.sourceCopy : isRemakeNoNarrationCopy(detectedCopy) ? "" : detectedCopy;
}

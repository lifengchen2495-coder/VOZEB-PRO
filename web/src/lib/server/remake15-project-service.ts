...
import { remakeProductionInputSnapshot, type RemakeProductionInputProject } from "@/lib/remake15-production-input";
import { renderRemakeCopyReport } from "./remake15-production-prompt";
...
import { createRemakeAnalysisTask, failRemakeAnalysisTask, findActiveRemakeAnalysisTask, getRemakeAnalysisTask, queueRemakeAnalysisTask, type RemakeAnalysisTask } from "@/lib/server/remake15-analysis-task-store";
import { remakeContactSheetDimensionError } from "@/lib/server/remake-contact-sheet-validation";
import { createRemakeProject, deleteRemakeProject, getRemakeProject, listRemakeProjectSummaries, mutateRemakeProject, RemakeProjectStoreError, updateRemakeProject } from "@/lib/server/remake15-project-store";
...
export async function updateRemakeProjectForUser(userId: string, id: string, value: unknown) {
    ...
    const productScript = sourceChanged || productInfoChanged || referencesChanged || hasOwn(input, "frames") ? "" : hasOwn(input, "productScript") ? cleanText(input.productScript, 100_000) : current.productScript;
    const productScriptChanged = productScript !== current.productScript;
    const storyboardScript = sourceChanged || productInfoChanged || referencesChanged || hasOwn(input, "frames") || productScriptChanged ? "" : hasOwn(input, "storyboardScript") ? cleanText(input.storyboardScript, 100_000) : current.storyboardScript;
    const storyboardScriptChanged = storyboardScript !== current.storyboardScript;
    ...
    let groups = sourceChanged
        ? emptyRemakeRangeGroups()
        : referencesChanged || imageModelChanged || productInfoChanged || productScriptChanged || storyboardScriptChanged
          ? invalidateRemakeImages(current.groups)
          : hasOwn(input, "groups")
            ? await normalizeEditableRemakeGroups({ userId, projectId: current.id, value: input.groups, current: current.groups, references, frames, modelSelection, productScript: storyboardScript })
            : current.groups;
    ...
    try {
        const saved = await updateRemakeProject(userId, next, current.revision);
...
export async function completeRemakeProductScriptsForUser(userId: string, id: string, input: { expectedRevision: number; productScript: string; storyboardScript: string }) {
    let previous: RemakeProject | undefined;
    const next = await mutateRemakeProject(userId, id, (current) => {
        if (current.revision !== input.expectedRevision) throw new RemakeProjectServiceError("脚本生成期间项目素材已变化，请重新生成", 409);
        const normalized = normalizeRemakeProjectWorkflow(current);
        previous = normalized;
        const groups = invalidateRemakeImages(normalized.groups);
        return withRevision(normalized, {
            productScript: cleanText(input.productScript, 100_000),
            storyboardScript: cleanText(input.storyboardScript, 100_000),
            groups,
            pipeline: deriveRemakePipeline({ ...normalized, groups }),
        });
    });
    if (!next) throw new RemakeProjectServiceError("复刻项目不存在", 404);
    if (previous) await cleanupReplacedMedia(userId, previous, next);
    return normalizeRemakeProjectWorkflow(next);
}
...
export async function completeRemakeProjectAnalysis(input: {
    ...
    const next = await mutateRemakeProject(input.task.userId, input.task.projectId, (current) => {
        ...
        if (copyBlocks.length !== REMAKE_COPY_BLOCK_COUNT) throw new RemakeProjectServiceError("视频分析必须返回完整的 4 个语义文案区间", 400);
...
export async function completeRemakeProductionForUser(userId: string, id: string, input: RemakeProductionCompletionInput) {
    ...
    const next = await mutateRemakeProject(userId, cleanText(id, 160), (current) => {
        ...
        if (normalized.frames.length !== REMAKE_FRAME_COUNT) throw new RemakeProjectServiceError("请先完成 12 帧视频分析", 409);
        const copy = normalizeRemakeCopyState(input.copy, normalized.copy);
        const copyBlocks = input.copyBlocks
        ...
        if (copyBlocks.length !== REMAKE_COPY_BLOCK_COUNT) throw new RemakeProjectServiceError("请提交完整的 4 个语义文案区间", 400);
        ...
        } else if (input.videoPrompts && (input.videoPrompts.length !== 1 || promptPatches.length !== 1)) {
            throw new RemakeProjectServiceError("请提交完整的 1 组视频提示词", 400);
        }
        ...
        if (!promptsReady && input.groupId === undefined) throw new RemakeProjectServiceError("1 组视频提示词尚未完整生成", 409);
        ...
        if (!copyReady) throw new RemakeProjectServiceError(noNarration ? "无口播视频的 4 个语义文案区间必须保持为空" : "4 个语义文案区间存在空内容", 409);
...
export async function assertRemakeImageGenerationsForUser(userId: string, project: Pick<HydratedRemakeProject, "id" | "references" | "groups" | "frames" | "modelSelection" | "storyboardScript">) {
    if (project.groups.length !== 1) throw new RemakeProjectServiceError("一组十二宫格生图任务不完整", 409);
    await Promise.all(
        project.groups.map(async (group) => {
            ...
            const replacement = await authoritativeRemakeImageGeneration({
                ...
                productScript: project.storyboardScript,
                requested: group.replacementGeneration,
                previous: group.replacementGeneration,
                stage: "replacement",
    ...
    return Promise.all(
        requested.map(async (group, index) => {
            ...
            const replacementGeneration = await authoritativeRemakeImageGeneration({
                ...
                frames: input.frames,
                productScript: input.productScript,
                requested: group.replacementGeneration,
                previous: previous.replacementGeneration,
                stage: "replacement",
            ...
            const imageGeneration =
                replacementGeneration.status === "completed"
                    ? await authoritativeRemakeImageGeneration({
                          ...
                          productScript: input.productScript,
                          requested: group.imageGeneration,
                          previous: previous.imageGeneration,
                          stage: "storyboard",
                          selectedModel: input.modelSelection.image,
                      })
                    : { status: "idle" as const, prompt: "", attemptNo: group.imageGeneration.attemptNo };
...
async function authoritativeRemakeImageGeneration(input: {
    ...
    const prompt = canonicalRemakeImagePrompt(input.stage, input.group, input.references, input.frames, input.productScript);
...
function authoritativeImageAsset(task: ImageTask, groupId: string): RemakeMediaAsset | undefined {
    ...
    return {
        ...
        originalName: `remake15-${groupId}.png`,
        bytes: result?.bytes,
        width: result?.width,
...
function canonicalRemakeImagePrompt(stage: "replacement" | "storyboard", group: RemakeRangeGroup, references: RemakeReferences, frames: RemakeFrame[], productScript: string) {
    if (!productScript.trim()) return "";
    if (stage === "replacement") return group.sourceContactSheet?.url ? remakeReplacementPersonPrompt(group.id, frames, Boolean(references.character?.url), productScript) : "";
    return group.replacementGeneration.result?.url && references.product?.url ? remakeStoryboardPrompt(group.id, frames, productScript) : "";
}
...
async function authoritativeRemakeVideoGeneration(input: {
    ...
    if (!task || task.userId !== input.userId || task.projectId !== input.projectId || task.generationSlotId !== `remake15-video:${input.group.id}` || task.prompt !== input.group.videoPrompt) {
    ...
    return { status: "completed", taskId: task.id, model, attemptNo, result: { ...result, originalName: `remake15-${input.group.id}-15s.mp4` } };
}
...
function deriveRemakePipeline(input: {
    ...
    const promptsReady = input.groups.length === 1 && input.groups.every((group) => Boolean(group.videoPrompt.trim()));
...
function normalizeVideoPromptPatches(value: RemakeVideoPromptInput[] | undefined) {
    ...
    for (const item of value) {
        ...
        const ordinal = integerInRange(source.groupOrdinal, 1, 1);
...
function withRevision(project: RemakeProject, patch: Partial<RemakeProject>): RemakeProject {
    return { ...project, ...patch, revision: project.revision + 1, updatedAt: new Date().toISOString() };
}

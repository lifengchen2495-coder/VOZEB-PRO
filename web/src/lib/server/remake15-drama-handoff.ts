...
import type { RemakeCopyBlock, RemakeFrame, RemakeProject } from "@/lib/server/remake15-project-contract";
...
import { getRemakeProjectForUser } from "@/lib/server/remake15-project-service";
...
export async function handoffRemakeProjectToDrama(userId: string, projectId: string) {
    ...
    const sourceHandoffId = `remake15-${project.id}`;
...
export function buildRemakeDramaCreateInput(project: RemakeProject, sourceHandoffId = `remake15-${project.id}`) {
        ...
        episodes: [
            {
                ...
                outline: "按原视频 12 个抽帧单元组织为 4 个连续生产区间。",
...
function buildShot(project: RemakeProject, block: RemakeCopyBlock, frames: RemakeFrame[], first: RemakeFrame, last: RemakeFrame, index: number): DramaShot {
    ...
    return {
        id: `remake15-shot-${project.id}-${block.ordinal}`,
        order: index + 1,
        ...
        utterances: copy ? [{ id: `remake15-utterance-${project.id}-${block.ordinal}`, order: 1, type: "voiceover", speaker: voiceover ? "旁白" : "", text: copy }] : [],
        ...
        duration,
        characterIds: voiceover ? ["remake15-narrator"] : [],
        propIds: [],
        clueIds: [],
...
function buildSourceAssets(project: RemakeProject): DramaSourceAsset[] {
    ...
    if (project.sourceVideo?.url) {
        assets.push({
            id: `remake15-source-video-${project.id}`,
            type: "video",
    ...
    if (project.sourceCopy.trim()) assets.push({ id: `remake15-source-copy-${project.id}`, type: "text", title: "原文案", textContent: project.sourceCopy });
    return assets;
}

function narratorCharacter(voice: RemakeProject["voice"]): DramaCharacter {
    return {
        id: "remake15-narrator",
        name: "旁白",
...
function assertHandoffReady(project: RemakeProject) {
    ...
    if (project.frames.length !== 12) throw new RemakeDramaHandoffError("请先完成 12 帧抽取", 409);
    if (project.copyBlocks.length !== 4) throw new RemakeDramaHandoffError("请先生成 4 个文案区间", 409);
}

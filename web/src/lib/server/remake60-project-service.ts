...
import { remakeReplacementPersonPrompt, remakeReplacementPromptReferences, remakeStoryboardPrompt, remakeStoryboardPromptReferences } from "@/lib/remake60-image-prompt";
import type { ImageTask } from "@/lib/server/image-task-store";
import { getStoredGenerationTaskRecord } from "@/lib/server/generation-task-store";
import { createRemakeAnalysisTask, failRemakeAnalysisTask, findActiveRemakeAnalysisTask, getRemakeAnalysisTask, queueRemakeAnalysisTask, type RemakeAnalysisTask } from "@/lib/server/remake60-analysis-task-store";
import { remakeContactSheetDimensionError } from "@/lib/server/remake-contact-sheet-validation";
import { createRemakeProject, deleteRemakeProject, getRemakeProject, listRemakeProjectSummaries, mutateRemakeProject, RemakeProjectStoreError, updateRemakeProject } from "@/lib/server/remake60-project-store";
...
export async function completeRemakeProjectAnalysis(input: {
    ...
    const next = await mutateRemakeProject(input.task.userId, input.task.projectId, (current) => {
        ...
        if (frames.length !== REMAKE_FRAME_COUNT || frames.some((frame, index) => frame.ordinal !== index + 1)) throw new RemakeProjectServiceError("视频分析必须返回完整的 48 个帧单元", 400);
        const timestamps = input.timestamps === undefined ? frames.map((frame) => frame.time) : normalizeRemakeTimestamps(input.timestamps);
        if (timestamps.length !== REMAKE_FRAME_COUNT) throw new RemakeProjectServiceError("视频分析必须返回完整的 48 个抽帧时间点", 400);
...
function authoritativeImageAsset(task: ImageTask, groupId: string): RemakeMediaAsset | undefined {
    ...
    return {
        ...
        originalName: `remake60-${groupId}.png`,
        bytes: result?.bytes,
        width: result?.width,
...
async function authoritativeRemakeVideoGeneration(input: {
    ...
    if (!task || task.userId !== input.userId || task.projectId !== input.projectId || task.generationSlotId !== `remake60-video:${input.group.id}` || task.prompt !== input.group.videoPrompt) {
    ...
    return { status: "completed", taskId: task.id, model, attemptNo, result: { ...result, originalName: `remake60-${input.group.id}-15s.mp4` } };
}
...
function normalizeEditableFrames(input: Record<string, unknown>, current: RemakeFrame[]) {
    ...
    if (incoming.length !== REMAKE_FRAME_COUNT || incoming.some((frame, index) => frame.ordinal !== index + 1)) throw new RemakeProjectServiceError("请提交完整的 48 个帧分析单元", 400);

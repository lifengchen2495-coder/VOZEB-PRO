...
export async function countLocalMediaReferences(storageKeys: string[]) {
    ...
    if (getDatabaseProvider() === "postgres") {
        ...
        const result = await postgresQuery<{ storage_key: string; total: number | string }>(
            `WITH requested AS (
                ...
                SELECT r.storage_key, count(*)::int
                FROM requested r
                JOIN remake_projects p ON position(r.storage_key in COALESCE(p.project_json::text, '')) > 0
                GROUP BY r.storage_key
                UNION ALL
                SELECT r.storage_key, count(*)::int
                FROM requested r
                ...
                SELECT r.storage_key, count(*)::int
                FROM requested r
                JOIN frame_remake_projects p ON position(r.storage_key in COALESCE(p.project_json::text, '')) > 0
                GROUP BY r.storage_key
                UNION ALL
                SELECT r.storage_key, count(*)::int
                FROM requested r
    ...
    const databases: unknown[] = await Promise.all([
        ...
        readJsonDataFile<unknown>("frame-remake-projects.json", {}),

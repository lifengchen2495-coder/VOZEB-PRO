...
import { createOmniProject, omniInputVersion, omniVideoReferences, omniVideoSize, type OmniInputPatch, type OmniMedia, type OmniProject, type OmniSegment } from "@/lib/omni-remake-contract";
...
import { normalizeVideoGenerationReferences, type VideoGenerationReference } from "@/lib/video-reference-contract";
...
export async function saveOmniProjectForUser(userId: string, id: string, revision: number, raw: unknown) {
    ...
    return mutateOmni(userId, id, (current) => {
        ...
        if (omniInputVersion(next) !== omniInputVersion(current)) {
            ...
            else next.segments = next.segments.map((segment) => ({ ...segment, prompt: "", promptZh: "", sourceClip: undefined, video: { status: "idle", attemptNo: segment.video.attemptNo } }));
...
export async function updateOmniPrompt(userId: string, id: string, revision: number, segmentId: string, prompt: string, promptZh: string) {
    if (!prompt.trim() || prompt.length > 30000 || promptZh.length > 30000) throw new OmniProjectError("片段提示词为空或过长");
    return mutateOmni(userId, id, (current) => {
        assertRevision(current, revision);
        assertIdle(current);
        if (!current.segments.some((segment) => segment.id === segmentId && segment.sourceClip)) throw new OmniProjectError("片段尚未准备好");
        return changed({
            ...current,
...
export async function startOmniOperation(userId: string, id: string, revision: number, kind: NonNullable<OmniProject["operation"]>["kind"]) {
    return mutateOmni(userId, id, (current) => {
        assertRevision(current, revision);
        assertIdle(current);
        return changed({ ...current, operation: { id: randomUUID(), kind, startedAt: new Date().toISOString() }, error: undefined });
    });
}
...
export async function reserveOmniVideo(userId: string, id: string, revision: number, segmentId: string) {
    const before = await getOmniProjectForUser(userId, id);
    const targetBefore = before.segments.find((segment) => segment.id === segmentId);
    if (!targetBefore) throw new OmniProjectError("片段不存在", 404);
    const settings = await getAuthSettings();
    const model = before.modelSelection.video || settings.defaultModels.videoModel;
    const channels = resolveLogicalModelCandidates(settings, "video", model).map(toSystemGenerationChannel);
    const desired = Math.ceil(targetBefore.duration);
    const generationDurationSeconds =
        targetBefore.video.status === "queued" && targetBefore.video.generationDurationSeconds
            ? targetBefore.video.generationDurationSeconds
            : channels.map((channel) => resolveUpstreamVideoDuration(desired, desired, { ...channel.capabilityProfile, durationRange: channel.advancedConfig?.durationRange })).find((duration) => duration >= targetBefore.duration && duration <= 10);
    if (!generationDurationSeconds) throw new OmniProjectError("所选模型不支持本片段时长，请选择支持视频参考且单段不超过 10 秒的模型");
    return mutateOmni(userId, id, (current) => {
        assertRevision(current, revision);
        if (current.operation) throw new OmniProjectError("项目正在处理中", 409);
        const target = current.segments.find((segment) => segment.id === segmentId);
        if (!target?.sourceClip?.url || !target.prompt.trim()) throw new OmniProjectError("请先准备片段与提示词", 409);
        if (target.video.status === "running") throw new OmniProjectError("该片段视频仍在生成", 409);
        const retryQueued = target.video.status === "queued";
        const attemptNo = retryQueued ? target.video.attemptNo : target.video.attemptNo + 1;
        const clientRequestId = retryQueued && target.video.clientRequestId ? target.video.clientRequestId : `omni-remake:${id}:${segmentId}:${randomUUID()}`;
        return changed({
            ...current,
            modelSelection: { ...current.modelSelection, video: model },
            mergedVideo: undefined,
            segments: current.segments.map((segment) => (segment.id === segmentId ? { ...segment, video: { status: "queued", attemptNo, clientRequestId, generationDurationSeconds } } : segment)),
        });
    });
}

export async function failOmniVideoSubmission(userId: string, id: string, segmentId: string, attemptNo: number, error: string) {
    return mutateOmni(userId, id, (current) =>
        changed({
            ...current,
            segments: current.segments.map((segment) => (segment.id === segmentId && segment.video.attemptNo === attemptNo && segment.video.status === "queued" ? { ...segment, video: { ...segment.video, status: "error", error } } : segment)),
        }),
    );
}

export async function validateOmniVideoRequest(input: { userId: string; projectId: string; slotId: string; prompt: string; references: unknown; seconds: unknown; attemptNo?: number; clientRequestId?: string; model?: string; size?: unknown }) {
    const project = await getOmniProjectForUser(input.userId, input.projectId);
    const segment = project.segments.find((item) => `omni-remake-video:${item.id}` === input.slotId);
    if (!segment?.sourceClip || !segment.prompt.trim() || project.operation) throw new OmniProjectError("Omni 片段尚未准备好", 409);
    if (input.prompt !== segment.prompt) throw new OmniProjectError("视频提示词与已保存内容不一致", 409);
    if (project.modelSelection.video && input.model !== project.modelSelection.video) throw new OmniProjectError("视频模型与已保存选择不一致", 409);
    if (segment.video.status !== "queued" || input.attemptNo !== segment.video.attemptNo || input.clientRequestId !== segment.video.clientRequestId) throw new OmniProjectError("Omni 生成尝试已经变化，请刷新", 409);
    const expected = normalizeVideoGenerationReferences(omniVideoReferences(project, segment));
    if (JSON.stringify(normalizeVideoGenerationReferences(input.references)) !== JSON.stringify(expected)) throw new OmniProjectError("Omni 参考素材与已保存片段不一致", 409);
    const durationSeconds = segment.video.generationDurationSeconds || Math.ceil(segment.duration);
    if (Number(input.seconds) !== durationSeconds) throw new OmniProjectError("Omni 视频时长与片段不一致");
    if (input.size !== omniVideoSize(project, segment)) throw new OmniProjectError("Omni 视频画幅与来源片段不一致");
    return { prompt: segment.prompt, durationSeconds, references: expected as VideoGenerationReference[] };
}

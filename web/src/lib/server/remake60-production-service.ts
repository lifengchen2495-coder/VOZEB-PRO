...
import { isRemakeNoNarrationCopy } from "@/lib/server/remake60-project-contract";
import { assertRemakeVideoPrompt, remakeProductionMessages, REMAKE_PRODUCTION_GROUP_IDS, renderRemakeCopyReport, type RemakeProductionPromptInput } from "@/lib/server/remake60-production-prompt";
import { assertRemakeImageGenerationsForUser, completeRemakeProductionForUser, getRemakeProjectForUser, remakeProductionInputVersion, RemakeProjectServiceError } from "@/lib/server/remake60-project-service";
import { buildRemakeProductionVisualBoards, RemakeProductionVisionError, requestRemakeProductionVisionPrompt, resolveRemakeProductionVisionProtocol } from "@/lib/server/remake60-production-vision-runtime";
...
const runtime = globalThis as typeof globalThis & { __vozebProRemake60Productions?: Map<string, PendingProduction> };
...
export async function buildRemakeProductionForUser(input: RemakeProductionRequest) {
    ...
    const pending = runtime.__vozebProRemake60Productions ??= new Map();
    ...
    try {
        for (const groupId of selectedGroups) {
            ...
            for (const candidate of candidates) {
                const idempotencyKey = systemAiIdempotencyKey("remake60-feishu-video-prompt", input.userId, project.id, remakeProductionInputVersion(project, [groupId]), groupId, candidate.channelId, candidate.upstreamModel);
...
async function assertProductionReady(userId: string, project: Awaited<ReturnType<typeof getRemakeProjectForUser>>) {
    if (!project.productScript.trim() || !project.storyboardScript.trim()) throw new RemakeProductionError("请先生成新产品脚本和四组分镜提示词", 409);

...
import { isRemakeNoNarrationCopy } from "@/lib/server/remake-product-project-contract";
import { assertRemakeVideoPrompt, remakeProductionMessages, REMAKE_PRODUCTION_GROUP_IDS, renderRemakeCopyReport, type RemakeProductionPromptInput } from "@/lib/server/remake-product-production-prompt";
import { assertRemakeImageGenerationsForUser, completeRemakeProductionForUser, getRemakeProjectForUser, remakeProductionInputVersion, RemakeProjectServiceError } from "@/lib/server/remake-product-project-service";
import { buildRemakeProductionVisualBoards, RemakeProductionVisionError, requestRemakeProductionVisionPrompt, resolveRemakeProductionVisionProtocol } from "@/lib/server/remake-product-production-vision-runtime";
...
const runtime = globalThis as typeof globalThis & { __vozebProRemakeProductProductions?: Map<string, PendingProduction> };
...
export async function buildRemakeProductionForUser(input: RemakeProductionRequest) {
    ...
    const pending = runtime.__vozebProRemakeProductProductions ??= new Map();
...
async function generateRemakeProduction(input: RemakeProductionRequest, project: Awaited<ReturnType<typeof getRemakeProjectForUser>>, selectedGroups: typeof REMAKE_PRODUCTION_GROUP_IDS[number][], expectedInputVersion: string) {
    ...
    try {
        ...
        throw new RemakeProductionError(toSafeGenerationErrorMessage(error, "产品图或最终十二宫格读取失败，无法执行真实视觉规划"), 502);
    }
    ...
    try {
        for (const groupId of selectedGroups) {
            ...
            for (const candidate of candidates) {
                const idempotencyKey = systemAiIdempotencyKey("remake-product-feishu-video-prompt", input.userId, project.id, remakeProductionInputVersion(project, [groupId]), groupId, candidate.channelId, candidate.upstreamModel);
...
async function assertProductionReady(userId: string, project: Awaited<ReturnType<typeof getRemakeProjectForUser>>) {
    if (!project.productInfo.trim()) throw new RemakeProductionError("请先填写新产品信息", 409);

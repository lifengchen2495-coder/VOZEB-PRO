...
export function CreateWorkspace({ commerceImage = false }: { commerceImage?: boolean }) {
    const basePath = commerceImage ? "/commerce/image" : "/create";
    const defaultSkillId = commerceImage ? "ecommerce-image" : undefined;
    ...
    const [selectedSkillId, setSelectedSkillId] = useState<string | undefined>(defaultSkillId);
    const [selectedModelIds, setSelectedModelIds] = useState<string[]>([]);
    const [smartPlanning, setSmartPlanning] = useState(true);
    const [creationMode, setCreationMode] = useState<"agent" | CreativeGenerationMode>(commerceImage ? "image" : "agent");
    ...
    }, [basePath, message, openAgentConversation, router]);
    ...
    useEffect(() => {
        ...
        router.replace(createConversationHref(agent.conversationId, basePath), { scroll: false });
    }, [agent.conversationId, basePath, router]);
                        ...
                        {showConversation ? (
                            ...
                            <div className={cn("mx-auto flex min-h-full w-full min-w-0 max-w-[1240px] flex-col items-center px-2.5 pb-3 sm:px-8 sm:pb-8", commerceImage ? "pt-14 sm:pt-12" : "pt-5 sm:pt-14 lg:pt-[10vh]")}>
                                ...
                                <div className="mt-2 flex w-full min-w-0 flex-wrap justify-center gap-1.5 sm:mt-3 sm:gap-2">
                                    {!commerceImage && <Link href="/frame-remake" className="inline-flex h-9 items-center gap-2 rounded-full border border-[#e3e7eb] bg-white px-3 text-sm font-medium text-[#343b44] transition hover:bg-[#f7f8fa] dark:border-[#343a42] dark:bg-[#181b20] dark:text-[#dce1e7]">原时长视频复刻</Link>}

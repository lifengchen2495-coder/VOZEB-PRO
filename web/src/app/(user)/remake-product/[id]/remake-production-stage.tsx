        ...
        async (groupId: string, announce = true) => {
            ...
            try {
                ...
                const task = await createServerVideoGenerationTask(generationConfig, prompt, remakeVideoReferenceImages(group, current.references), [], remakeVideoAudioReferences(current), {
                    ...
                    clientRequestId,
                    attemptNo,
                    generationSlotId: `remake-product-video:${group.id}`,
                });
            ...
            <div className="mx-auto w-full max-w-[1480px] px-3 py-4 sm:px-5 sm:py-5">
                <div className="flex min-w-0 flex-col gap-3 border-b border-border pb-4 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap items-end gap-2">
                        ...
                        {project.mergedVideo?.url ? <Button href={`/api/remake-product/projects/${encodeURIComponent(project.id)}/merge`} icon={<Download className="size-4" />}>下载 1 分钟成片</Button> : null}
                ...
                {project.mergedVideo?.url ? <section className="mb-4 rounded-lg border border-border p-3" aria-label="1 分钟换品不换人成片"><h3 className="mb-3 text-sm font-semibold">1 分钟换品不换人成片</h3><video className="mx-auto max-h-[560px] max-w-full rounded-md bg-black" src={browserReadableMediaUrl(project.mergedVideo.url)} controls playsInline preload="metadata" /></section> : null}
...
function productionPrerequisites(project: RemakeProject) {
    ...
    if (!remakeImagesReady(project)) missing.push("4 组换品十二宫格");
    if (!project.productInfo.trim()) missing.push("新产品信息");
...
function remakeVideoClientRequestId(project: RemakeProject, group: RemakeRangeGroup, model: string) {
    ...
    return `remake-product-video:${group.id}:${stableTextHash(input)}`;
}

...
export function remakeVideoInputVersion(project: RemakeProject, groupId: string) {
    ...
    return JSON.stringify([
        ...
        project.modelSelection.video, assetIdentity(project.references.product), assetIdentity(project.references.audio),
    ]);
}
...
export function invalidateRemakeImages(project: RemakeProject, patch: RemakeWorkspacePatch): RemakeWorkspacePatch {
    return {
        ...patch,
        groups: project.groups.map((group) => ({
            ...group,

...
import { remakeProductionInputSnapshot } from "@/lib/remake-product-production-input";
...
export function RemakeWorkspace() {
    ...
    if (!project || loadError) return <WorkspaceError message={loadError || "复刻项目不存在"} onBack={() => router.push("/remake-product")} onRetry={() => void load()} />;
    ...
    return (
        <main className="flex h-full min-h-0 flex-col overflow-hidden bg-background text-foreground" data-remake-workspace aria-label="1 分钟换品不换人工作区">
...
function RemakeFlowNavigation({ stage, analysisReady, imagesReady, productionReady, onChange }: { stage: RemakeFlowStage; analysisReady: boolean; imagesReady: boolean; productionReady: boolean; onChange: (stage: RemakeFlowStage) => void }) {
    ...
    return (
        <nav className="flex h-12 shrink-0 items-stretch overflow-x-auto border-b border-border bg-card px-1 sm:justify-center sm:px-3" aria-label="1 分钟换品不换人流程">

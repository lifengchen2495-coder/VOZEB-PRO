...
export async function mergeRemakeVideos(id: string, revision: number) {
    return projectFromPayload(await requestPayload(`/api/remake-product/projects/${encodeURIComponent(id)}/merge`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "text/event-stream" }, body: JSON.stringify({ revision }) }));
}

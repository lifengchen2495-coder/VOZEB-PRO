...
export default function RemakeProjectsPage() {
    ...
    const create = async () => {
        ...
        try {
            ...
            setCreateOpen(false);
            setTitle("");
            router.push(`/remake-product/${encodeURIComponent(project.id)}`);
        } catch (reason) {
            ...
            <div className="mx-auto w-full max-w-7xl px-2 py-2 sm:px-6 sm:py-8">
                <header className="flex items-end justify-between gap-3 border-b border-border pb-3 sm:gap-5 sm:pb-6">
                    <div className="min-w-0">
                        ...
                        <p className="mt-1.5 text-xs leading-5 text-muted-foreground sm:mt-2 sm:text-sm">共 {total} 个项目 · 48 分镜 · 换品不换人 · 1 分钟成片</p>
                    </div>

...
export async function downloadRemakeProductionBundle(project: RemakeProject): Promise<RemakeProductionExportResult> {
    ...
    files.push(
        ...
        { name: "分析/12镜头解析.md", data: project.analysis.raw || "" },
        { name: "分析/新产品信息.txt", data: project.productInfo },
        { name: "分析/新产品-12分镜脚本.md", data: project.productScript },
        { name: "提示词/1-12分镜提示词.md", data: project.storyboardScript },
        { name: "分析/抽帧时间点.txt", data: project.analysis.timestamps.map(formatTimestamp).join(",") },
        { name: "分析/12镜头结构.json", data: JSON.stringify(project.frames, null, 2) },
        {
            name: "提示词/生图-全部.txt",
            data: project.groups
    ...
    saveAs(zip, `${safeExportFileName(project.title || project.id)}-15秒复刻生产包.zip`);
...
function assertCompleteProductionBundle(project: RemakeProject) {
    ...
    if (project.frames.length !== 12 || project.frames.some((frame, index) => frame.ordinal !== index + 1 || frame.analysisStatus !== "available" || !frame.frameUrl)) missing.push("12 张抽帧");
    ...
    if (project.copyBlocks.length !== 4 || project.copyBlocks.some((block, index) => block.ordinal !== index + 1 || (noNarration ? Boolean(block.sourceText.trim() || block.text.trim()) : !block.sourceText.trim() || !block.text.trim()))) {
        missing.push(noNarration ? "无口播分镜预处理" : "4 个文案区间");
    }
    if (

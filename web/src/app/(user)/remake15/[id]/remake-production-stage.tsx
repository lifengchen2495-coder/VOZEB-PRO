...
import { remakeEffectiveVideoPromptInstructions, normalizeRemakeVideoPromptInstructions } from "@/lib/remake15-video-prompt-instructions";
...
export function RemakeProductionStage({
    ...
    const promptsReady = project.groups.length === 1 && project.groups.every((group) => group.videoPrompt.trim());
        ...
        async (groupId: string, announce = true) => {
            ...
            try {
                ...
                const task = await createServerVideoGenerationTask(generationConfig, prompt, remakeVideoReferenceImages(group, current.references), [], remakeVideoAudioReferences(current), {
                    ...
                    clientRequestId,
                    attemptNo,
                    generationSlotId: `remake15-video:${group.id}`,
                });
            ...
            <div className="mx-auto w-full max-w-[1480px] px-3 py-4 sm:px-5 sm:py-5">
                <div className="flex min-w-0 flex-col gap-3 border-b border-border pb-4 lg:flex-row lg:items-end lg:justify-between">
                    <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">12 个分镜 · 1 条 15 秒竖屏视频</p>
                    </div>
                ...
                <div className="flex items-center gap-2 border-t border-border pt-4 text-xs text-muted-foreground">
                    ...
                    {productionReady ? "15 秒视频及生产素材已就绪" : "生成 1 条 15 秒视频后，可下载视频及完整生产包"}
                </div>
            </div>
        </section>
    );
...
function productionPrerequisites(project: RemakeProject) {
    ...
    const noNarration = isRemakeNoNarrationCopy(project.sourceCopy);
    if (!project.sourceVideo?.url || project.analysis.status !== "completed" || project.analysis.mode !== "video" || project.frames.length !== 12 || project.frames.some((frame) => frame.analysisStatus !== "available" || !frame.frameUrl)) missing.push("完整视频理解与 12 镜头解析");
    if (!isRemakeCopyPlanReady(project)) missing.push(noNarration ? "无口播分镜预处理" : "4 个语义文案区间");
...
function remakeVideoClientRequestId(project: RemakeProject, group: RemakeRangeGroup, model: string) {
    ...
    return `remake15-video:${group.id}:${stableTextHash(input)}`;
}

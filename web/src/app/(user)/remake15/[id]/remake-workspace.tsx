...
import { remakeProductionInputSnapshot } from "@/lib/remake15-production-input";
...
export function RemakeWorkspace() {
    ...
    const editingLocked = isRemakeAnalysisActive(analyzing, task?.status) || buildingScript || buildingProduction || buildingGroupIds.length > 0 || Boolean(videoActive) || handoffPending;
    ...
    if (!project || loadError) return <WorkspaceError message={loadError || "复刻项目不存在"} onBack={() => router.push("/remake15")} onRetry={() => void load()} />;
    ...
    return (
        <main className="flex h-full min-h-0 flex-col overflow-hidden bg-background text-foreground" data-remake-workspace aria-label="15 秒电商复刻工作区">
                    ...
                    <div className="flex h-full min-h-0 min-w-0 flex-col">
                        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border bg-background px-3 py-2 sm:px-4">
                            <div className="min-w-0">
                                ...
                                <div className="truncate text-sm font-semibold">来源视频理解与 12 镜头解析</div>
                            </div>
...
function RemakeFlowNavigation({ stage, analysisReady, imagesReady, productionReady, onChange }: { stage: RemakeFlowStage; analysisReady: boolean; imagesReady: boolean; productionReady: boolean; onChange: (stage: RemakeFlowStage) => void }) {
    ...
    return (
        <nav className="flex h-12 shrink-0 items-stretch overflow-x-auto border-b border-border bg-card px-1 sm:justify-center sm:px-3" aria-label="15 秒复刻流程">

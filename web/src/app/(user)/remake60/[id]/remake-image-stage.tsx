...
export function RemakeImageStage({
    ...
    buildingScript: boolean;
    onProductChange: (patch: { productInfo?: string; storyboardScript?: string }) => void;
    onBuildScript: () => Promise<unknown>;
    ...
    const startStage = useCallback(
        async (groupId: string, stage: ImageStage, announce = true) => {
            ...
            if (!current.productInfo.trim() || !current.productScript.trim() || !current.storyboardScript.trim()) return message.warning("请先填写产品信息并生成新产品脚本与分镜提示词");
            ...
            const inputVersion = remakeGroupInputVersion(group, current.references, stage, current.storyboardScript);
        ...
        for (const group of project.groups) {
            for (const stage of ["replacement", "storyboard"] as const) {
                ...
                if ((generation.status === "queued" || generation.status === "running") && generation.taskId && model) {
                    ...
                    void waitForGroupTask(stage, group.id, generation.taskId, prompt, remakeGroupInputVersion(group, project.references, stage, project.storyboardScript), model);
            ...
            <div className="mx-auto w-full max-w-[1480px] px-3 py-4 sm:px-5 sm:py-5">
                <div className="flex min-w-0 flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-end sm:justify-between">
                    <div className="min-w-0">
                        ...
                        <h2 className="mt-1 text-lg font-semibold">新产品脚本与十二宫格重绘</h2>
                        <p className="mt-1 text-sm text-muted-foreground">结合产品信息生成 48 分镜脚本与四组十二宫格提示词，审阅后分别生成四组最终分镜图。</p>
                    </div>
                    <div className="flex shrink-0 flex-wrap items-center gap-2">
                        <ModelPicker
                            ...
                            placeholder="选择生图模型"
                        />
                        <Tag className="!m-0">{completedCount} / 1 已完成</Tag>
                    </div>
                </div>
                ...
                <section className="grid gap-3 border-b border-border py-4" aria-label="新产品脚本">
                    ...
                    {project.productScript ? <details><summary className="cursor-pointer text-sm font-medium">查看新产品 48 分镜脚本</summary><Input.TextArea readOnly value={project.productScript} autoSize={{ minRows: 4, maxRows: 12 }} className="!mt-2" /></details> : null}
                    {project.storyboardScript ? <label className="grid gap-2 text-sm font-medium">四组分镜提示词 · 审阅后生图<Input.TextArea value={project.storyboardScript} disabled={disabled || generationActive} autoSize={{ minRows: 7, maxRows: 16 }} onChange={(event) => onProductChange({ storyboardScript: event.target.value })} /></label> : null}
                </section>
                ...
                <div className="grid items-start gap-3 py-4">
                    {project.groups.map((group) => (
                        <RemakeGroupCard key={group.id} project={project} group={group} references={project.references} disabled={disabled || Boolean(uploadingKey) || !referencesReady || !scriptReady} onGenerate={() => void startGroup(group)} />
                    ))}
                </div>

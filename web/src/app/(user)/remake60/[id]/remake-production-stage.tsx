...
export function RemakeProductionStage({
    ...
    const sharedBusy = merging || building || buildingGroupIds.length > 0 || promptBuildPendingRef.current.size > 0 || batchPromptBuildPendingRef.current || videoActive;
        ...
        async (groupId: string, announce = true) => {
            ...
            try {
                ...
                const task = await createServerVideoGenerationTask(generationConfig, prompt, remakeVideoReferenceImages(group, current.references), [], remakeVideoAudioReferences(current), {
                    ...
                    clientRequestId,
                    attemptNo,
                    generationSlotId: `remake60-video:${group.id}`,
                });
    ...
    const updateInstructions = (groupId: string, value: string) => {
        if (sharedBusy) return;
        try {
            normalizeRemakeVideoPromptInstructions(value);
        } catch (error) {
            message.error(error instanceof Error ? error.message : "生成指令不正确");
            return;
        }
        emitGroupChange(groupId, { videoPromptInstructions: value });
    };
    const saveInstructions = async () => {
        if (!(await onFlush())) throw new Error("生成指令尚未保存，请先处理保存错误后重试");
    };
    ...
    const buildPrompts = async (groupId?: string) => {
        ...
        try {
            if (!(await onFlush())) {
                message.error("生成指令尚未保存，请先处理保存错误后重试");
                return;
            }
    ...
    const exportBundle = async () => {
        ...
        try {
            ...
            message.success("1分钟换品换人复刻生产包已下载");
        } catch (reason) {
    ...
    async function mergeVideos() {
        ...
        try {
            ...
            onMerged(saved);
            message.success("四段视频已合并为 1 分钟成片");
        } catch (error) {
            ...
            <div className="mx-auto w-full max-w-[1480px] px-3 py-4 sm:px-5 sm:py-5">
                <div className="flex min-w-0 flex-col gap-3 border-b border-border pb-4 lg:flex-row lg:items-end lg:justify-between">
                    <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">48 个分镜 · 4 条 15 秒竖屏视频 · 合并 1 分钟</p>
                    </div>
                    <div className="flex shrink-0 flex-wrap items-end gap-2">
                        ...
                        <Button loading={merging} disabled={!productionReady || sharedBusy} onClick={() => void mergeVideos()}>合并 1 分钟视频</Button>
                        {project.mergedVideo?.url ? <Button href={`/api/remake60/projects/${encodeURIComponent(project.id)}/merge`} icon={<Download className="size-4" />}>下载 1 分钟成片</Button> : null}
                    ...
                    <div className="min-w-0">
                        ...
                        {noNarration ? (
                            <>
                                ...
                                <p className="mt-2 text-xs leading-5 text-muted-foreground">每条视频 Prompt 保留对应的 12 个连续分镜动作，不添加口播、配音或音频引用。</p>
                            </>
                        ) : (
                            <>
                                <Segmented
                ...
                <div className="grid min-w-0 gap-4 py-4 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
                    ...
                    <section className="min-w-0" aria-label="Seedance 视频提示词与视频">
                        <div className="mb-2.5 flex flex-wrap items-center justify-between gap-2">
                            <div className="flex min-w-0 items-center gap-2 text-sm font-semibold"><Video className="size-4 text-muted-foreground" />15 秒视频提示词与成片</div>
                        ...
                        <div className="grid gap-3">
                            {project.groups.map((group) => (
                                <VideoGroupCard
                                    ...
                                    onInstructionsChange={(value) => updateInstructions(group.id, value)}
                                    onSaveInstructions={saveInstructions}
                                    building={isPromptBuilding(group.id)}
                                    promptDisabled={merging || isPromptBuilding(group.id) || !prerequisites.ready || isVideoActive(group) || startingGroupsRef.current.has(group.id)}
                                    disabled={merging || !group.videoPrompt.trim() || group.imageGeneration.status !== "completed" || !group.imageGeneration.result?.url || !project.references.product?.url || startingGroupsRef.current.has(group.id)}
                ...
                <div className="flex items-center gap-2 border-t border-border pt-4 text-xs text-muted-foreground">
                    ...
                    {productionReady ? "4 条视频及生产素材已就绪" : "生成 4 条 15 秒视频后，可下载视频及完整生产包"}
                </div>
            </div>
        </section>
    );
...
function productionPrerequisites(project: RemakeProject) {
    ...
    if (!remakeImagesReady(project)) missing.push("两步换品十二宫格");
...
function remakeVideoClientRequestId(project: RemakeProject, group: RemakeRangeGroup, model: string) {
    const input = [project.id, group.id, model, group.videoPromptInstructions?.trim() || "", group.videoPrompt, group.imageGeneration.result?.storageKey || group.imageGeneration.result?.url, project.references.product?.storageKey || project.references.product?.url, project.references.character?.storageKey || project.references.character?.url, project.references.audio?.storageKey || project.references.audio?.url].join("\n");
    return `remake60-video:${group.id}:${stableTextHash(input)}`;
}

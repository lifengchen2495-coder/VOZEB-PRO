...
import { remakeProductionInputSnapshot } from "@/lib/remake60-production-input";

import { buildRemakeProductScript, buildRemakeProduction, getRemakeProject, getRemakeTask, handoffRemakeProject, RemakeConflictError, RemakeRequestError, saveRemakeProject, startRemakeAnalysis, uploadRemakeVideo } from "../remake-api";
...
import { editRemakeCopyBlock, hasRemakePatch, invalidateRemakeProduction, invalidateRemakeProductScript, isRemakeAnalysisActive, mergeEditablePatch, mergeRemakeConcurrentResult, mergeRemakeVideoProgress, remakeVideoInputVersion, rebaseRemakeConflict, recoveredFlowStage, type RemakePendingVideoProgress, type RemakeWorkspacePatch } from "./remake-workspace-state";
...
export function RemakeWorkspace() {
    ...
    const editingLocked = mergingVideo || isRemakeAnalysisActive(analyzing, task?.status) || buildingScript || buildingProduction || buildingGroupIds.length > 0 || Boolean(videoActive) || handoffPending;
    ...
    const updateFrame = useCallback(
        (patch: Partial<RemakeFrame>) => {
            ...
            queuePatch(
                invalidateRemakeProductScript(current, invalidateRemakeProduction(current, {
    ...
    const updateReference = useCallback(
        (key: ReferenceKey, asset?: RemakeMediaAsset) => {
            ...
            queuePatch({ references, groups, productScript: "", storyboardScript: "" });
        },
        [queuePatch],
    );

    const updateProduct = useCallback((patch: Pick<RemakeWorkspacePatch, "productInfo" | "storyboardScript">) => {
        const current = projectRef.current;
        if (!current || editingLockedRef.current) return;
        queuePatch(invalidateRemakeProductScript(current, {
            ...patch,
            ...(patch.storyboardScript !== undefined ? { productScript: current.productScript } : {}),
        }));
    }, [queuePatch]);
    ...
    const buildProductScript = async () => {
        if (editingLockedRef.current) return;
        editingLockedRef.current = true;
        setBuildingScript(true);
        try {
            if (!(await flushSave())) return message.warning("请先处理保存错误，再生成新产品脚本");
            const current = projectRef.current;
            if (!current) return;
            const next = await buildRemakeProductScript(projectId, current.revision);
            applyConcurrentProject(next);
            message.success("新产品脚本与分镜提示词已生成，请审阅后开始生图");
        } catch (reason) {
            if (reason instanceof RemakeConflictError) {
                try {
                    applyConcurrentProject(await getRemakeProject(projectId));
                } catch (refreshReason) {
                    message.error(refreshReason instanceof Error ? refreshReason.message : "项目最新版本加载失败");
                }
            }
            message.error(reason instanceof Error ? reason.message : "新产品脚本生成失败");
        } finally {
    ...
    const buildProductionContent = async (groupId?: string) => {
        ...
        try {
            ...
            message.success(groupId ? `分镜 ${groupId} 的 Prompt 已生成` : "四段视频提示词已生成");
        } catch (reason) {
    ...
    if (!project || loadError) return <WorkspaceError message={loadError || "复刻项目不存在"} onBack={() => router.push("/remake60")} onRetry={() => void load()} />;
    ...
    return (
        <main className="flex h-full min-h-0 flex-col overflow-hidden bg-background text-foreground" data-remake-workspace aria-label="1 分钟换品换人复刻工作区">
            ...
            <div className="min-h-0 min-w-0 flex-1 overflow-hidden">
                ...
                {flowStage === "images" ? (
                    <RemakeImageStage
                        ...
                        onBuildScript={buildProductScript}
...
function RemakeFlowNavigation({ stage, analysisReady, imagesReady, productionReady, onChange }: { stage: RemakeFlowStage; analysisReady: boolean; imagesReady: boolean; productionReady: boolean; onChange: (stage: RemakeFlowStage) => void }) {
    ...
    return (
        <nav className="flex h-12 shrink-0 items-stretch overflow-x-auto border-b border-border bg-card px-1 sm:justify-center sm:px-3" aria-label="1 分钟换品换人复刻流程">

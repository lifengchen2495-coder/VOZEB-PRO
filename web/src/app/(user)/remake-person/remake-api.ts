...
export async function mergeRemakeVideos(id: string, revision: number) {
    return projectFromPayload(await requestPayload(`/api/remake-person/projects/${encodeURIComponent(id)}/merge`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ revision }) }));
}

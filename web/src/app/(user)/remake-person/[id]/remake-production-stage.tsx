            ...
            <div className="mx-auto w-full max-w-[1480px] px-3 py-4 sm:px-5 sm:py-5">
                <div className="flex min-w-0 flex-col gap-3 border-b border-border pb-4 lg:flex-row lg:items-end lg:justify-between">
                    <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">48 个分镜 · 4 条 15 秒竖屏视频</p>
                    </div>
                    <div className="flex shrink-0 flex-wrap items-end gap-2">
                        ...
                        {project.mergedVideo?.url ? <Button href={`/api/remake-person/projects/${encodeURIComponent(project.id)}/merge`} icon={<Download className="size-4" />}>下载 1 分钟成片</Button> : null}
                        ...
                        <div className="grid gap-3">
                            {project.groups.map((group) => (
                                <VideoGroupCard
                                    ...
                                    disabled={!group.videoPrompt.trim() || group.imageGeneration.status !== "completed" || !group.imageGeneration.result?.url || !project.references.background?.url || startingGroupsRef.current.has(group.id)}
                ...
                {project.mergedVideo?.url ? <section className="mb-4 rounded-lg border border-border p-3" aria-label="1 分钟换人不换品成片"><h3 className="mb-3 text-sm font-semibold">1 分钟换人不换品成片</h3><video className="mx-auto max-h-[560px] max-w-full rounded-md bg-black" src={browserReadableMediaUrl(project.mergedVideo.url)} controls playsInline preload="metadata" /></section> : null}
...
function remakeVideoClientRequestId(project: RemakeProject, group: RemakeRangeGroup, model: string) {
    const input = [project.id, group.id, model, group.videoPrompt, group.imageGeneration.result?.storageKey || group.imageGeneration.result?.url, project.references.background?.storageKey || project.references.background?.url, project.references.character?.storageKey || project.references.character?.url, project.references.audio?.storageKey || project.references.audio?.url].join("\n");

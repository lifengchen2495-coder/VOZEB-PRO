...
export async function downloadRemakeProductionBundle(project: RemakeProject): Promise<RemakeProductionExportResult> {
    ...
    files.push(
        ...
        { name: "分析/48镜头结构.json", data: JSON.stringify(project.frames, null, 2) },
        {
            name: "提示词/生图-全部.txt",
            data: project.groups
                .map((group) => `=== 分镜 ${group.id} · 保留原产品换人 ===

${group.imageGeneration.prompt}`)
                .join("\n\n"),
        },
        ...project.groups.flatMap((group) => [
            { name: `提示词/生图-${String(group.ordinal).padStart(2, "0")}-${group.id}-保留原产品换人.txt`, data: group.imageGeneration.prompt },
        ]),
        {
    ...
    saveAs(zip, `${safeExportFileName(project.title || project.id)}-换人不换品生产包.zip`);

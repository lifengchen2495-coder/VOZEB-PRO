...
export function DramaVideoPromptInstructions({ project, episode, disabled, onBusyChange }: { project: DramaProject; episode: DramaEpisode; disabled?: boolean; onBusyChange: (busy: boolean) => void }) {
    ...
    const run = async (generate: boolean) => {
        ...
        try {
            ...
            const response = await fetch("/api/drama/analyze", {
                ...
                body: JSON.stringify({ ...input, phase: "video-prompts", requestId: `drama-video-prompts:${project.id}:${episode.id}:${nanoid()}` }),
                signal: controller.signal,
            });
            syncUserPointsFromHeaders(response.headers, "system");
            const payload = (await response.json().catch(() => ({}))) as { data?: DramaVideoPromptAnalysis; msg?: string };
            if (!response.ok || !payload.data) throw new Error(payload.msg || "视频提示词生成失败");
            const store = useDramaStore.getState();
            ...
            store.applyVideoPromptAnalysis(project.id, episode.id, payload.data, expectedInput);
    ...
    return <div className="mb-4 space-y-2">
        <VideoPromptInstructionEditor
            ...
            onSave={() => run(false)}
            onGenerate={() => run(true)}
            generationDisabled={hasActiveMedia || episode.reviewStatus !== "visual_ready" || !episode.shots.length}
        />
        <p className="text-xs leading-6 text-muted-foreground">指令用于当前项目所有剧集，重新生成仅更新当前集。人物、场景及分镜图会保留，旧视频和合成结果需重新生成。{hasActiveMedia ? "当前有媒体任务进行中，完成后可重新生成提示词。" : dirty ? "当前显示的镜头提示词仍来自之前的指令。" : ""}</p>
    </div>;
}

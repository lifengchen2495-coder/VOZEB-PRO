...
const DRAMA_AGENT_STAGE_GUIDES: Record<DramaProjectStage, { label: string; prompts: Array<{ label: string; prompt: string }> }> = {
    ...
    story: {
        label: "故事分析协作",
        prompts: [
            { label: "核对故事分析", prompt: "根据已导入的剧本原文，核对故事分析中的核心冲突和世界规则，指出遗漏或误读。原文未交代的内容标为未交代。" },
            { label: "检查原文依据", prompt: "核对故事分析与原剧本的事实是否一致，区分明确写出的事实和推断，不补写情节。" },
        ],
    },
    characters: {
        label: "人物小传协作",
    ...
    script: {
        label: "剧本协作",
        prompts: [
            { label: "解读剧本", prompt: "阅读当前集原剧本，概括故事、人物关系、关键事件和结尾悬念。只分析已有内容，不重写正文，也不要求用户手填分析表单。" },

...
export async function PATCH(request: Request, context: Context) {
    const user = await getCurrentUser(request);
    if (!user) return omniResponse(null, "请先登录", 401);
    const body = await readJsonBodyResult<Record<string, unknown>>(request, 128 * 1024);
    if (!body.ok) return omniResponse(null, body.message, body.status);
    try {
        return omniResponse(await saveOmniProjectForUser(user.id, (await context.params).id, Number(body.data?.revision), body.data));
    } catch (error) {
        return omniError(error);
    }
}

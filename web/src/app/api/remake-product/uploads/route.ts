...
import { cleanupTemporaryVideoUpload, RemakeUploadInputError, REMAKE_VIDEO_UPLOAD_MAX_BYTES, streamRequestToTemporaryVideo } from "./upload-stream";
...
export async function POST(request: Request) {
    ...
    const limit = await checkRateLimit(`remake-product-upload:${user.id}`, UPLOAD_RATE_LIMIT);
    ...
    try {
        ...
        const asset = await writeReferenceMediaFile(temporary.filePath, "video", temporary.mimeType, true, {
            ownerUserId: user.id,
            source: "remake-product-source-upload",
            originalName,
            projectId,
        ...
        if (error instanceof RemakeUploadInputError) return response(error.status, null, error.message);

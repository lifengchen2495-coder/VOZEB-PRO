...
export const REMAKE_VIDEO_UPLOAD_MAX_BYTES = 200 * 1024 * 1024;
...
export class RemakeUploadInputError extends Error {
    constructor(
        message: string,
        readonly status = 400,
    ) {
        super(message);
        this.name = "RemakeUploadInputError";
    }
}

export async function streamRequestToTemporaryVideo(request: Request, maxBytes = REMAKE_VIDEO_UPLOAD_MAX_BYTES): Promise<TemporaryVideoUpload> {
    const limit = Math.max(1, Math.floor(maxBytes));
    const declaredMimeType = normalizeContentType(request.headers.get("content-type"));
    if (!VIDEO_MIME_TYPES.has(declaredMimeType)) throw new RemakeUploadInputError("仅支持 MP4、MOV 或 WebM 视频", 415);
    assertContentLength(request.headers.get("content-length"), limit);
    if (!request.body) throw new RemakeUploadInputError("请选择要上传的视频");

    const directory = await mkdtemp(join(tmpdir(), "vozeb-pro-remake-upload-"));
    ...
    try {
        ...
        if (!streamed.bytes) throw new RemakeUploadInputError("上传的视频不能为空");
        const detected = await fileTypeFromBuffer(streamed.prefix);
        const detectedMimeType = detected?.mime.toLowerCase() || "";
        if (!VIDEO_MIME_TYPES.has(detectedMimeType)) throw new RemakeUploadInputError("视频文件内容格式不正确", 415);
        if (detectedMimeType !== declaredMimeType) throw new RemakeUploadInputError("视频文件类型与 Content-Type 不一致", 415);
        succeeded = true;
    ...
    try {
        while (true) {
            ...
            if (total > maxBytes) {
                ...
                throw new RemakeUploadInputError("单个视频不能超过 200MB", 413);
            }
...
function assertContentLength(value: string | null, maxBytes: number) {
    ...
    if (!/^\d+$/.test(value.trim())) throw new RemakeUploadInputError("Content-Length 不正确");
    const bytes = Number(value);
    if (!Number.isSafeInteger(bytes)) throw new RemakeUploadInputError("Content-Length 不正确");
    if (bytes > maxBytes) throw new RemakeUploadInputError("单个视频不能超过 200MB", 413);
    if (bytes === 0) throw new RemakeUploadInputError("上传的视频不能为空");
}

...
import { getRemakeProjectForUser as getRemake15ProjectForUser, RemakeProjectServiceError as Remake15ProjectServiceError } from "@/lib/server/remake15-project-service";

import { getRemakeProjectForUser as getRemake60ProjectForUser, RemakeProjectServiceError as Remake60ProjectServiceError } from "@/lib/server/remake60-project-service";
import { getRemakeProjectForUser as getRemakeProductProjectForUser, RemakeProjectServiceError as RemakeProductProjectServiceError } from "@/lib/server/remake-product-project-service";
...
export async function POST(request: Request) {
    ...
    const response = await withGenerationConcurrencyLimit(
        ...
        async () => {
            ...
            const fixedRemake = [
                { projectPrefix: "remake60-", slotPrefix: "remake60-video:", get: getRemake60ProjectForUser },
                { projectPrefix: "remake-product-", slotPrefix: "remake-product-video:", get: getRemakeProductProjectForUser },
                { projectPrefix: "remake-person-", slotPrefix: "remake-person-video:", get: getRemakePersonProjectForUser },
            ].find((entry) => remakeProjectId.startsWith(entry.projectPrefix) || remakeSlotId.startsWith(entry.slotPrefix));
            ...
            if (isRemake15VideoRequest) {
                if (!remakeProjectId.startsWith("remake15-") || remakeSlotId !== "remake15-video:1-12") return NextResponse.json({ error: "15 秒复刻视频任务的项目或分镜标识不完整" }, { status: 400 });
                if (body.config?.videoSeconds !== undefined && Number(body.config.videoSeconds) !== 15) return NextResponse.json({ error: "15 秒复刻视频仅支持生成 15 秒视频" }, { status: 400 });
                try {
                    const project = await getRemake15ProjectForUser(user.id, remakeProjectId);
                    const group = project.groups.find((item) => item.id === "1-12");
                    if (!group) return NextResponse.json({ error: "15 秒复刻视频任务的分镜组不存在" }, { status: 400 });
                    if (!group.videoPrompt.trim()) return NextResponse.json({ error: "复刻视频提示词尚未生成，请先生成并保存提示词" }, { status: 409 });
                    if (requestedPrompt !== group.videoPrompt) return NextResponse.json({ error: "复刻视频提示词与已保存原文不一致，请刷新项目后重试" }, { status: 409 });
                    prompt = group.videoPrompt;
                } catch (error) {
                    if (error instanceof Remake15ProjectServiceError) return NextResponse.json({ error: error.message }, { status: error.status });
                    throw error;
                }
            }
            try {

...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
    ...
    const body = await readJsonBodyResult<{ revision?: number; action?: string; stageScope?: FrameRemakeWorkflowStage }>(request, 4096);
    ...
    try {
        const project = await controlFrameRemakeAutomation(user.id, (await context.params).id, body.data.revision, body.data.action, body.data.stageScope);
